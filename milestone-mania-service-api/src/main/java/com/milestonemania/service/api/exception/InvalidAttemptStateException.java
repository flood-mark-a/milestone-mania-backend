...
public class InvalidAttemptStateException extends RuntimeException {
   ...
   * @return InvalidAttemptStateException with descriptive message
   */
  public static InvalidAttemptStateException invalidState(
      Long attemptId, AttemptStatus currentStatus, AttemptStatus requiredStatus) {
    return new InvalidAttemptStateException(
        String.format(
            "Attempt %d is in %s state, but %s is required",
