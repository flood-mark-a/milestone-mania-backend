...
public class InsufficientMilestonesException extends RuntimeException {
   ...
   */
  public static InsufficientMilestonesException notEnoughMilestones(
      int availableCount, int requiredCount) {
    return new InsufficientMilestonesException(
        String.format(
            "Insufficient milestones available: found %d, need %d", availableCount, requiredCount));
  }
}
