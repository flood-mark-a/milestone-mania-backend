...
public class GameController {
  ...
  @Operation(
      ...
      description =
          "Creates a new timeline ordering game with a unique slug and optional player name")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
            ...
            content =
                @Content(
                    ...
                    schema =
                        @Schema(
                            implementation =
                                com.milestonemania.controller.dto.response.ErrorResponse.class))),
        @ApiResponse(
            responseCode = "503",
  ...
  public ResponseEntity<GameAttemptDto> createNewGame(
    ...
    logger.info(
        "Creating new game - CorrelationId: {}, PlayerName: {}",
        correlationId,
    ...
    logger.info(
        "New game created - CorrelationId: {}, GameSlug: {}, AttemptId: {}",
        correlationId,
        gameAttempt.getGameSlug(),
  ...
  public ResponseEntity<GameAttemptDto> startExistingGame(
    ...
    correlationId = CorrelationIdUtil.getOrGenerateCorrelationId(correlationId);
    logger.info(
        "Starting existing game - CorrelationId: {}, Slug: {}, PlayerName: {}",
        correlationId,
        slug,
        request.getPlayerName());
    ...
    logger.info(
        "Game started - CorrelationId: {}, Slug: {}, AttemptId: {}",
        correlationId,
        slug,
        gameAttempt.getAttemptId());

    return ResponseEntity.ok().header("X-Correlation-ID", correlationId).body(gameAttempt);
  }
  ...
  public ResponseEntity<SubmitAttemptResponse> submitAttempt(
    ...
    correlationId = CorrelationIdUtil.getOrGenerateCorrelationId(correlationId);
    logger.info(
        "Submitting attempt - CorrelationId: {}, AttemptId: {}, MilestoneIds: {}",
        correlationId,
        request.getAttemptId(),
        request.getOrderedMilestoneIds());

    SubmitAttemptResponse response = gameService.submitAttempt(request);

    logger.info(
        "Attempt submitted - CorrelationId: {}, IsCorrect: {}, AttemptNumber: {}",
        correlationId,
        response.isCorrect(),
        response.getAttemptNumber());

    return ResponseEntity.ok().header("X-Correlation-ID", correlationId).body(response);
  }
  ...
  public ResponseEntity<GameDto> getGameBySlug(
    ...
    logger.info(
        "Game retrieved - CorrelationId: {}, Slug: {}, GameId: {}",
        correlationId,
        slug,
        game.getId());

    return ResponseEntity.ok().header("X-Correlation-ID", correlationId).body(game);
  }
}
